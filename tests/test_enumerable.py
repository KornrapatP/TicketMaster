...
def test_set_factory_market(protocol):
    ...
    assert collection.tokenOfOwnerByIndex(fan1, 0) == 10
    with brownie.reverts():
        collection.tokenOfOwnerByIndex(fan1, 1)

    market.primaryBuy(collection, 1, fan1, {"from": fan1, "value": 10})
    assert collection.tokenOfOwnerByIndex(fan1, 0) == 10
    assert collection.tokenOfOwnerByIndex(fan1, 1) == 11
    collection.transferFrom(fan1, fan2, 10, {"from": fan1})
    assert collection.tokenOfOwnerByIndex(fan1, 0) == 11
    assert collection.tokenOfOwnerByIndex(fan2, 0) == 10
    with brownie.reverts():
        collection.tokenOfOwnerByIndex(fan1, 1)
